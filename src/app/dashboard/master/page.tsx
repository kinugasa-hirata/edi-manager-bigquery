...
const GROUP_NAMES: Record<string, string> = {
  A: 'M90S', B: '300NP', C: '100G20', D: '950X01',
}
...
const GROUP_COLORS: Record<string, string> = {
  'M90S':   'bg-blue-50 text-blue-700',
  '300NP':  'bg-green-50 text-green-700',
  '100G20': 'bg-red-50 text-red-700',
  '950X01': 'bg-purple-50 text-purple-700',
}
...
export default function MasterPage() {
  ...
  async function fetchData() {
    ...
    try {
      const [pRes, lRes] = await Promise.all([
        databases.listDocuments(DB_ID, COLLECTIONS.PRODUCT_MASTER,  [Query.orderAsc('sort_order'), Query.limit(100)]),
      ...
      setLots(lRes.documents as unknown as Lot[])
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Sheet1: Product master (group codes)
      addLog('商品マスタを登録中...')
      ...
      for (let colIdx = 0; colIdx < 4; colIdx++) {
        ...
        const codes = raw1
          .map((row: any) => row[colIdx]?.toString().trim())
          .filter(Boolean)
        for (let i = 0; i < codes.length; i++) {
          ...
          const data = {
            ...
            group_name:   gname,
            weight_g:     null,
            sort_order:   colIdx * 100 + i,
          }
          try {
            await databases.createDocument(DB_ID, COLLECTIONS.PRODUCT_MASTER, code, data)
          } catch (err: any) {
            if (err?.code === 409) {
              await databases.updateDocument(DB_ID, COLLECTIONS.PRODUCT_MASTER, code, data)
            }
          }
        }
      ...
      for (const row of raw3.slice(1)) {
        ...
        if (!code || isNaN(w)) continue
        try {
          await databases.updateDocument(DB_ID, COLLECTIONS.PRODUCT_MASTER, code, { weight_g: w })
          weightCount++
        } catch {}
      }
      addLog(`  重量更新: ${weightCount} 件`)
      ...
      for (let i = 1; i < raw2.length; i++) {
        ...
        const data  = {
          lot_id:     lotId,
          lot_label:  LOT_LABELS[lotId] ?? lotId,
          start_from: start.toISOString(),
          end_at:     end.toISOString(),
          sort_order: i,
        }
        try {
          await databases.createDocument(DB_ID, COLLECTIONS.LOT_DEFINITIONS, lotId, data)
        } catch (err: any) {
          if (err?.code === 409) {
            await databases.updateDocument(DB_ID, COLLECTIONS.LOT_DEFINITIONS, lotId, data)
          }
        }
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="divide-y divide-gray-50">
              {GROUP_ORDER.map(gname => {
                const gProducts = products
                  .filter(p => p.group_name === gname)
                  .sort((a, b) => a.sort_order - b.sort_order)
                if (gProducts.length === 0) return null
                return (
                  <div key={gname} className="px-5 py-3">
                    <div className="flex items-center gap-2 mb-2">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${GROUP_COLORS[gname] ?? 'bg-gray-100 text-gray-600'}`}>
                        {gname}
                      </span>
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="text-gray-400">
                            <th className="text-left py-1 pr-6 font-medium">品番コード</th>
                            <th className="text-right py-1 font-medium">重量(g)</th>
                          </tr>
                        </thead>
                  ...
                  return (
                    <tr key={lotId} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-2 text-gray-500">
                        {new Date(lot.start_from).toLocaleDateString('ja-JP')}
                      </td>
                      <td className="px-5 py-2 text-gray-500">
                        {new Date(lot.end_at).toLocaleDateString('ja-JP')}
                      </td>
                    </tr>
                  )
                })}
